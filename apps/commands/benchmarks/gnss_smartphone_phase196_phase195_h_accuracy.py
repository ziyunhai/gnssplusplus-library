#!/usr/bin/env python3
"""Pre-truth H evaluator for the frozen Phase195 native output.

``--verify`` reads only the Phase196 manifest and static source/result metadata.
``--evaluate`` is authorization-gated and performs one candidate/truth read,
delegating the unchanged exact-key, Haversine, and percentile math to the
sealed Phase189 kernel with the pinned Phase76/Phase74 helpers.
"""
...
FOCUSED_TESTS = ROOT / "tests/test_smartphone_phase196_phase195_h_accuracy.py"
...
PHASE195_RESULT_SHA = "a7086efbf6697d450c88263bcc2e7edc33a32e15c2d271adc61055c38ea6d7fb"
PHASE195_MANIFEST_SHA = "df68dd4758196154a3a23be7c1a471e8e012a2d0997b57632261734e4896a362"
PHASE195_CANDIDATE_SHA = "97e0e372b472e0186ca05b4ec6552e8300970b6cfff6eaf5b8f4f7399c6ecadc"

MANIFEST = ROOT / "docs/use_cases/records/smartphone_r5_phase196_phase195_h_accuracy_manifest_v1.json"
AUTHORIZATION = ROOT / "docs/use_cases/records/smartphone_r5_phase196_phase195_h_accuracy_authorization_v1.json"
RESULT = ROOT / "docs/use_cases/records/smartphone_r5_phase196_phase195_h_accuracy_result_v1.json"
SCHEMA = "smartphone-r5-phase196-phase195-h-accuracy-manifest.v1"
RESULT_SCHEMA = "smartphone-r5-phase196-phase195-h-accuracy-result.v1"


class Phase196Error(ValueError):
    """Raised when the frozen Phase196 contract fails closed."""


def fail(message: str) -> Phase196Error:
    return Phase196Error(message)
...
def read_json(path: Path, label: str) -> dict[str, Any]:
    ...
    except (OSError, UnicodeDecodeError, json.JSONDecodeError, Phase196Error) as exc:
...
def load_kernel() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("phase189_kernel_phase196", KERNEL_PATH)
...
def verify_manifest(path: Path = MANIFEST) -> dict[str, Any]:
    manifest = read_json(path, "Phase196 manifest")
    expected = {
        "schema_version": SCHEMA,
        "phase": 196,
        "execution_label": "Luna Max",
        "status": "frozen-before-phase196-truth-read",
        "route": ROUTE,
    ...
    safe_payload_path(candidate.get("path"), "opaque_solution_output.csv", "/phase195-h-native-phase194-imu-noise-v1/", "candidate")
    ...
    if candidate.get("sha256") != PHASE195_CANDIDATE_SHA:
        raise fail("candidate is not the sealed Phase195 output")
    ...
    if truth.get("archive_crc32") != "fe212a4d" or truth.get("role") != "development/train; not heldout or leaderboard proof":
        raise fail("truth provenance/role changed")
    ...
    authority_entry(authority, "phase189_kernel", KERNEL_PATH, KERNEL_SHA)
    ...
    authority_entry(
        authority,
        "phase195_result",
        ROOT / "docs/use_cases/records/smartphone_r5_phase195_h_native_phase194_imu_noise_result_v1.json",
        PHASE195_RESULT_SHA,
    )
    authority_entry(
        authority,
        "phase195_manifest",
        ROOT / "docs/use_cases/records/smartphone_r5_phase195_h_native_phase194_imu_noise_manifest_v1.json",
        PHASE195_MANIFEST_SHA,
    )
...
def score_payloads(
    ...
    *,
    ...
    """Delegate unchanged Phase189 scoring; route is explicit and payloads are read once."""
    try:
        return load_kernel().score_payloads(
            candidate_path,
            candidate_pin,
            truth_path,
            truth_pin,
...
def evaluate(
    ...
    manifest = verify_manifest(manifest_path)
    authorization = read_json(authorization_path, "Phase196 authorization")
    if (
        authorization.get("schema_version") != "smartphone-r5-phase196-phase195-h-accuracy-authorization.v1"
        or authorization.get("allow_truth_read") is not True
    ):
        raise fail("truth authorization is missing or mismatched")
    if authorization.get("manifest_sha256") != static_hash(manifest_path, "Phase196 manifest"):
    ...
    score = score_payloads(
        ROOT / manifest["candidate"]["path"], manifest["candidate"],
        ROOT / manifest["truth"]["path"], manifest["truth"], p76, p74,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 0
    except Phase196Error as exc:
        print(f"phase196: {exc}", file=sys.stderr)
        return 2
