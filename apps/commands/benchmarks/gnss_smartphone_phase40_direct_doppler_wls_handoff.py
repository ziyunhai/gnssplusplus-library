#!/usr/bin/env python3
"""Truth-free Phase40 direct raw Doppler-WLS handoff matrix.

The candidate consumes the existing in-process
``FGOProblem.doppler_velocity_wls_estimates`` sequence directly as the
Android IMU velocity/heading seed.  It deliberately does not run a
GNSS-first optimizer and never opens truth, MATLAB, validation, holdout, or
precomputed-coordinate inputs.
"""
...
FREEZE_SHA256 = "544089e32a7f7c0ca96ee6cbbfd7634145aebe288fe32a947ef1f67d5c07f52e"
BINARY = ROOT / "build/apps/gnss_fgo_imu_no_base"
DEFAULT_OUTPUT = ROOT / "output/smartphone-r5/phase40-direct-doppler-wls-handoff-v1"
RESULT_NAME = "phase40_structural_seal.json"
MANIFEST_NAME = "phase40_structural_seal.manifest.json"
FAILURE_NAME = "phase40_structural_failure.json"
...
CANDIDATE_FLAG = "--native-direct-doppler-wls-handoff"
MAX_SPEED_MPS = 70.0
MAX_CLOCK_RATE_MPS = 2000.0
...
class Phase40Error(ValueError):
    """Raised when a truth-free Phase40 contract cannot be proven."""


def _fail(message: str) -> Phase40Error:
    return Phase40Error(message)
...
def verify_freeze() -> dict[str, Any]:
    if sha256(FREEZE) != FREEZE_SHA256:
        raise _fail("Phase40 freeze hash changed")
    freeze = load_json(FREEZE, "Phase40 freeze")
    if freeze.get("schema_version") != "smartphone-r5-phase40-direct-doppler-wls-handoff-freeze.v1":
        raise _fail("Phase40 freeze schema mismatch")
    if freeze.get("status") != "frozen-before-implementation-and-solver":
        raise _fail("Phase40 freeze is not pre-implementation")
    objective = freeze.get("objective", {})
    if objective.get("candidate_flag") != CANDIDATE_FLAG:
        raise _fail("Phase40 candidate flag changed")
    if tuple(objective.get("structural_routes", ())) != ROUTES:
        raise _fail("Phase40 route order changed")
    if objective.get("truth_free") is not True:
        raise _fail("Phase40 truth-free contract changed")
    recipe = freeze.get("base_recipe", {})
    if tuple(recipe.get("flags", ())) != BASE_FLAGS or recipe.get("candidate_addition") != CANDIDATE_FLAG:
        raise _fail("Phase40 base recipe changed")
    if recipe.get("flag_off_default_unchanged") is not True or recipe.get("phase31_champion_unchanged") is not True:
        raise _fail("Phase40 default/champion contract changed")
    gates = freeze.get("predeclared_numeric_gates", {})
    for key in ("direct_valid_count_reported", "propagated_valid_count_reported", "rejected_count_reported",
                "coverage_count_equals_epoch_count", "all_estimates_valid", "all_estimates_finite"):
        if gates.get(key) is not True:
            raise _fail(f"Phase40 coverage gate changed: {key}")
    if gates.get("velocity_norm_max_mps") != MAX_SPEED_MPS or gates.get("velocity_norm_comparison") != "<=":
        raise _fail("Phase40 velocity gate changed")
    if gates.get("clock_rate_abs_max_mps") != MAX_CLOCK_RATE_MPS or gates.get("edge_hold_max_s") != 1.0:
        raise _fail("Phase40 clock/edge gate changed")
    gnss = gates.get("gnss_first", {})
    if gnss.get("attempted") is not False or gnss.get("positions_clocks_copied") != 0:
        raise _fail("Phase40 GNSS-first gate changed")
    output = gates.get("output", {})
    expected_output = {
        "finite": True,
        "converged": True,
        "raw_key_exact": True,
    ...
    if any(output.get(key) != value for key, value in expected_output.items()):
        raise _fail("Phase40 output gate changed")
    tdcp = gates.get("tdcp", {})
    if tdcp.get("factors_built_positive") is not True or tdcp.get("factors_inserted_equals_built") is not True or tdcp.get("inserted_positive") is not True:
        raise _fail("Phase40 TDCP gate changed")
    if gates.get("repeat_runs") != 2 or gates.get("byte_identical_submission_and_summary") is not True:
        raise _fail("Phase40 repeat gate changed")
    forbidden = "\n".join(str(item).lower() for item in freeze.get("forbidden_actions", []))
    for term in ("ground_truth", "validation", "holdout", "mat", "precomputed", "alternate trajectory",
                 "edge hold", "truth", "phase31", "kaggle", "execute matrix"):
        if term not in forbidden:
            raise _fail(f"Phase40 forbidden-input contract is incomplete: {term}")
    expected_hashes = freeze.get("input_contract", {}).get("sha256", {})
    if set(expected_hashes) != set(ROUTES):
        raise _fail("Phase40 raw input hash route set changed")
    return freeze
...
def input_paths(route: str) -> dict[str, Path]:
    ...
    root = (PHASE25_RAW_ROOT / route_path / phone
            ...
            else PHASE37_RAW_ROOT / route_path / phone / "inputs")
...
def verify_inputs(freeze: dict[str, Any], route: str) -> dict[str, Any]:
    ...
    if not isinstance(expected, dict):
        raise _fail(f"Phase40 input hash record missing: {route}")
    reports: dict[str, Any] = {}
    for key, path in input_paths(route).items():
        filename = RAW_FILES[key]
        actual = sha256(path)
        if actual != expected.get(filename):
            raise _fail(f"Phase40 raw input hash changed: {route}/{filename}")
...
def validate_candidate_summary(path: Path, dataset_id: str, raw_epoch_count: int) -> dict[str, Any]:
    summary = load_json(path, "Phase40 candidate summary")
    if summary.get("dataset_id") != dataset_id or summary.get("truth_used") is not False:
        raise _fail(f"candidate identity/truth contract failed: {dataset_id}")
    if summary.get("production_default_changed") is not False or summary.get("native_direct_doppler_wls_handoff") is not True:
        raise _fail(f"candidate default/direct marker failed: {dataset_id}")
    if summary.get("native_pdc_imu_tdcp_no_bridge") is not True or summary.get("native_quality_anchor") is not True or summary.get("native_pdc_state_bridge") is not False:
        raise _fail(f"candidate base flags failed: {dataset_id}")
    ...
    if gnss.get("attempted") is not False or gnss.get("handoff_mode") != "direct-doppler-wls":
        raise _fail(f"candidate GNSS-first bypass/direct marker failed: {dataset_id}")
    if int(gnss.get("positions_clocks_copied", -1)) != 0:
        raise _fail(f"candidate position/clock copy gate failed: {dataset_id}")
    if int(gnss.get("coverage_epochs", -1)) != raw_epoch_count or gnss.get("coverage_all_epochs") is not True:
        raise _fail(f"candidate WLS coverage epoch gate failed: {dataset_id}")
    direct = int(gnss.get("direct_valid_count", -1))
    propagated = int(gnss.get("propagated_valid_count", -1))
    rejected = int(gnss.get("rejected_count", -1))
    if direct < 0 or propagated < 0 or rejected < 0 or direct + propagated != raw_epoch_count or rejected != 0:
        raise _fail(f"candidate WLS direct/propagated/rejected gate failed: {dataset_id}")
    if int(gnss.get("valid_count", -1)) != raw_epoch_count:
        raise _fail(f"candidate WLS valid count failed: {dataset_id}")
    if int(gnss.get("nonfinite_count", -1)) != 0 or int(gnss.get("over_70_mps_count", -1)) != 0:
        raise _fail(f"candidate WLS finite/70 m/s gate failed: {dataset_id}")
    if int(gnss.get("clock_rate_over_2000_mps_count", -1)) != 0:
        raise _fail(f"candidate WLS clock-rate gate failed: {dataset_id}")
    if _finite_number(gnss.get("max_velocity_norm_mps"), "candidate WLS max velocity") > MAX_SPEED_MPS:
        raise _fail(f"candidate WLS velocity exceeds 70 m/s: {dataset_id}")
    if _finite_number(gnss.get("max_clock_rate_abs_mps"), "candidate WLS max clock rate") > MAX_CLOCK_RATE_MPS:
        raise _fail(f"candidate WLS clock rate exceeds 2000 m/s: {dataset_id}")
    if _finite_number(gnss.get("edge_hold_max_s"), "candidate edge-hold bound") > 1.0:
        raise _fail(f"candidate edge-hold bound changed: {dataset_id}")
    ...
    if graph.get("converged") is not True or _finite_number(graph.get("final_cost"), "candidate final cost") > _finite_number(graph.get("initial_cost"), "candidate initial cost") + 1e-9:
        raise _fail(f"candidate IMU graph convergence/cost gate failed: {dataset_id}")
    ...
    if int(raw.get("raw_epoch_keys", -1)) != raw_epoch_count or int(raw.get("target_epochs", -1)) != raw_epoch_count - 1 or int(raw.get("exact_solution_epochs", -1)) != raw_epoch_count - 1 or int(raw.get("interpolated_epochs", -1)) != 0 or int(raw.get("edge_hold_epochs", -1)) != 0 or int(raw.get("unresolved_epochs", -1)) != 0:
        raise _fail(f"candidate exact raw-key gate failed: {dataset_id}")
    return {"gnss_first": gnss, "graph": graph, "epochs": epochs, "tdcp_contract": tdcp, "raw_utc_key_contract": raw}
...
def run_native(paths: dict[str, Path], dataset_id: str, run_dir: Path, candidate: bool, allow_failure: bool = False) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {"candidate": candidate, "command": command, "return_code": process.returncode, "wall_seconds": wall, "log": {"path": relative(log), "sha256": sha256(log)}}
    if process.returncode != 0:
        if not allow_failure:
            raise _fail(f"native solver failed ({process.returncode}): {dataset_id}")
        report["failure_classification"] = "direct_wls_coverage_or_solver_fail_closed"
    ...
    report.update(artifact_report(submission, summary, dataset_id, raw_keys[1:], len(raw_keys)))
    return report
...
def _candidate_route_report(freeze: dict[str, Any], output_root: Path, route: str) -> dict[str, Any]:
    ...
    first = run_native(paths, route, native_root / "run1", candidate=True, allow_failure=True)
    if first["return_code"] != 0:
        return {"dataset_id": route, "source": "phase40-direct-doppler-wls-repeat", "raw_inputs": raw_inputs, "raw_epoch_count": len(raw_keys), "target_epoch_count": len(raw_keys) - 1, "run1": first, "repeat_byte_identical": False, "truth_open_count": 0, "mat_read_or_generated": False}
    ...
    return {"dataset_id": route, "source": "phase40-direct-doppler-wls-repeat", "raw_inputs": raw_inputs, "raw_epoch_count": len(raw_keys), "target_epoch_count": len(raw_keys) - 1, "run1": first, "run2": repeat, "repeat_byte_identical": True, "truth_open_count": 0, "mat_read_or_generated": False}
...
def _flag_off_report(freeze: dict[str, Any], output_root: Path, route: str) -> dict[str, Any]:
    ...
    raw_keys = read_raw_epoch_keys(paths["device_gnss"])
    baseline = freeze["flag_off_identity_contract"].get("expected_baselines", {}).get(route)
    if not isinstance(baseline, dict):
        raise _fail(f"Phase40 flag-off baseline missing: {route}")
    first = run_native(paths, route, root / "run1", candidate=False)
    ...
    return {"dataset_id": route, "source": "phase40-flag-off-frozen-baseline-control", "raw_inputs": raw_inputs, "expected_baseline": baseline, "run1": first, "run2": repeat, "repeat_byte_identical": True, "frozen_baseline_byte_identical": True, "truth_open_count": 0, "mat_read_or_generated": False}
...
def seal(output_root: Path = DEFAULT_OUTPUT, run_matrix: bool = False) -> dict[str, Any]:
    ...
    if run_matrix:
        if output_root.exists() and any(output_root.iterdir()):
            raise _fail(f"refusing to overwrite nonempty Phase40 output: {output_root}")
        output_root.mkdir(parents=True, exist_ok=True)
        for route in ROUTES:
            try:
                report = _candidate_route_report(freeze, output_root, route)
                candidate_routes[route] = report
                if report["run1"]["return_code"] != 0:
                    failures.append(f"candidate {route}: direct WLS coverage/solver failed closed")
                    break
            except (OSError, Phase40Error) as exc:
        ...
        for route in ROUTES:
            ...
            raw_inputs = verify_inputs(freeze, route)
            raw_keys = read_raw_epoch_keys(paths["device_gnss"])
            first = artifact_report(root / "run1" / "submission.csv", root / "run1" / "summary.json", route, raw_keys[1:], len(raw_keys))
            repeat = artifact_report(root / "run2" / "submission.csv", root / "run2" / "summary.json", route, raw_keys[1:], len(raw_keys))
            if first["submission"]["sha256"] != repeat["submission"]["sha256"] or first["summary"]["sha256"] != repeat["summary"]["sha256"]:
                raise _fail(f"candidate repeat differs: {route}")
            candidate_routes[route] = {"dataset_id": route, "source": "phase40-direct-doppler-wls-repeat", "raw_inputs": raw_inputs, "raw_epoch_count": len(raw_keys), "target_epoch_count": len(raw_keys) - 1, "run1": first, "run2": repeat, "repeat_byte_identical": True, "truth_open_count": 0, "mat_read_or_generated": False}
        for route in ROUTES:
            flag_off_routes[route] = _flag_off_report(freeze, output_root, route)

    if failures:
        failure = {
            "schema_version": "smartphone-r5-phase40-structural-failure.v1",
            "phase": 40,
            "status": "sealed-structural-no-go",
        ...
        raise _fail("Phase40 structural matrix failed; truth remains sealed")

    result = {
        "schema_version": "smartphone-r5-phase40-structural-seal.v1",
        "phase": 40,
        "status": "sealed-truth-free-structural-pass",
        "decision": "structural-go-development-only; truth remains sealed",
        "freeze": {"path": relative(FREEZE), "sha256": FREEZE_SHA256},
        "native": {"binary": {"path": relative(BINARY), "sha256": sha256(BINARY)}, "base_flags": list(BASE_FLAGS), "candidate_flag": CANDIDATE_FLAG, "inputs_only": ["raw device_gnss.csv", "raw device_imu.csv", "broadcast brdc.nav"], "ground_truth_used": False, "precomputed_result_coordinates_used_for_inference": False, "edge_hold_or_alternate_trajectory_used": False, "production_default_changed": False, "phase31_champion_mutated": False},
        "candidate_routes": candidate_routes,
        "flag_off_routes": flag_off_routes,
        "structural_gate": {
            "routes_passed": len(candidate_routes),
            "routes_total": len(ROUTES),
            "direct_propagated_rejected_coverage": all(route["run1"]["projection"]["gnss_first"]["rejected_count"] == 0 for route in candidate_routes.values()),
            "gnss_first_attempted_false": all(route["run1"]["projection"]["gnss_first"]["attempted"] is False for route in candidate_routes.values()),
            "positions_clocks_copied_zero": all(route["run1"]["projection"]["gnss_first"]["positions_clocks_copied"] == 0 for route in candidate_routes.values()),
            ...
            "raw_key_exact_no_edge_hold": all(route["run1"]["projection"]["raw_utc_key_contract"]["unresolved_epochs"] == 0 and route["run1"]["projection"]["raw_utc_key_contract"]["edge_hold_epochs"] == 0 for route in candidate_routes.values()),
            "repeat_byte_identical": all(route["repeat_byte_identical"] for route in candidate_routes.values()),
            "flag_off_frozen_baseline_byte_identical": all(route.get("frozen_baseline_byte_identical", False) for route in flag_off_routes.values()),
            "truth_used_false": True,
        },
        "truth_policy": {"truth_open_count": 0, "ground_truth_materialized": False, "validation_truth_open_count": 0, "holdout_truth_open_count": 0, "mat_read_or_generated": False, "token_or_kaggle_access": False, "post_truth_tuning": False},
    }
    atomic_json(output_root / RESULT_NAME, result)
    atomic_json(output_root / MANIFEST_NAME, {"schema_version": "smartphone-r5-phase40-structural-seal-manifest.v1", "status": result["status"], "result": {"path": relative(output_root / RESULT_NAME), "sha256": sha256(output_root / RESULT_NAME)}, "freeze": result["freeze"], "candidate_routes": list(candidate_routes), "flag_off_routes": list(flag_off_routes), "route_count": len(ROUTES), "repeat_runs": 2, "truth_open_count": 0, "ground_truth_materialized": False, "validation_truth_open_count": 0, "holdout_truth_open_count": 0, "mat_read_or_generated": False, "atomic_publish": True})
    return result
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.operation == "verify-freeze":
            ...
            print(json.dumps({"status": result["status"], "routes": len(result["candidate_routes"]), "truth_open_count": 0}, sort_keys=True))
    except (OSError, Phase40Error, ValueError, KeyError, TypeError, json.JSONDecodeError) as exc:
        print(f"phase40: {exc}", file=sys.stderr)
        return 2
    return 0
