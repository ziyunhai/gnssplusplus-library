...
MANIFEST = RECORDS / 'smartphone_r5_phase207_h_accuracy_manifest_v1.json'
AUTH = RECORDS / 'smartphone_r5_phase207_h_accuracy_authorization_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase207_h_accuracy_result_v1.json'
...
def verify():
    m = read(MANIFEST)
    require(m['phase'] == 207 and m['route'] == ROUTE, 'wrong experiment')
    ...
    structural = read(ROOT / a['phase206_result']['path'])
    ...
    require(structural['graph']['phase205_bias_density']['enabled'], 'density disabled')
...
def evaluate():
    m = verify()
    auth = read(AUTH)
    require(auth == {'phase': 207, 'allow_truth_read': True,
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_207', path)
    ...
    result = {'phase': 207, 'execution_label': 'primary agent',
