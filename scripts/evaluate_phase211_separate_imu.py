...
MANIFEST = RECORDS / 'smartphone_r5_phase211_h_accuracy_manifest_v1.json'
AUTH = RECORDS / 'smartphone_r5_phase211_h_accuracy_authorization_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase211_h_accuracy_result_v1.json'
...
def verify():
    m = read(MANIFEST)
    require(m['phase'] == 211 and m['route'] == ROUTE, 'wrong experiment')
    ...
    structural = read(ROOT / a['phase210_result']['path'])
    ...
    require(structural['graph']['phase209_separate_imu'] == {'requested': True, 'enabled': True, 'motion_factors': 3139, 'bias_factors': 3139, 'inclusive_samples': 166469}, 'separate factor counts changed')
...
def evaluate():
    m = verify()
    auth = read(AUTH)
    require(auth == {'phase': 211, 'allow_truth_read': True,
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_211', path)
    ...
    result = {'phase': 211, 'execution_label': 'primary agent',
