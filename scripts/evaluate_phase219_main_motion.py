...
MANIFEST = RECORDS / 'smartphone_r5_phase219_h_accuracy_manifest_v1.json'
AUTH = RECORDS / 'smartphone_r5_phase219_h_accuracy_authorization_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase219_h_accuracy_result_v1.json'
...
def verify():
    m = read(MANIFEST)
    require(m['phase'] == 219 and m['route'] == ROUTE, 'wrong experiment')
    ...
    structural = read(ROOT / a['phase218_result']['path'])
...
def evaluate():
    m = verify()
    auth = read(AUTH)
    require(auth == {'phase': 219, 'allow_truth_read': True,
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_219', path)
    ...
    result = {'phase': 219, 'execution_label': 'primary agent',
