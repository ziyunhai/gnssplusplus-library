...
MANIFEST = RECORDS / 'smartphone_r5_phase223_h_accuracy_manifest_v1.json'
AUTH = RECORDS / 'smartphone_r5_phase223_h_accuracy_authorization_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase223_h_accuracy_result_v1.json'
ROUTE = '2021-08-24-20-32-us-ca-mtv-h/pixel5'
BASELINE = 1.2689788175187473
...
def verify():
    m = read(MANIFEST)
    require(m['phase'] == 223 and m['route'] == ROUTE, 'wrong experiment')
    ...
    structural = read(ROOT / a['phase222_result']['path'])
    ...
    require(all(structural['unchanged_vs_phase218'].values()), 'baseline composition changed')
...
def evaluate():
    m = verify()
    auth = read(AUTH)
    require(auth == {'phase': 223, 'allow_truth_read': True,
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_223', path)
    ...
    result = {'phase': 223, 'execution_label': 'primary agent',
