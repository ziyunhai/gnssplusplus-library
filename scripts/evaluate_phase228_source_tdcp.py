...
MANIFEST = RECORDS / 'smartphone_r5_phase228_h_accuracy_manifest_v1.json'
AUTH = RECORDS / 'smartphone_r5_phase228_h_accuracy_authorization_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase228_h_accuracy_result_v1.json'
ROUTE = '2021-08-24-20-32-us-ca-mtv-h/pixel5'
BASELINE = 1.2680574850266653
...
def verify():
    m = read(MANIFEST)
    require(m['phase'] == 228 and m['route'] == ROUTE, 'wrong experiment')
    ...
    structural = read(ROOT / a['phase227_result']['path'])
    ...
    require(all(structural['comparison_vs_phase222'][k] for k in ('epochs', 'imu_measurement_noise', 'imu_utc_fallback_offset', 'output_contract', 'raw_utc_key_contract')), 'baseline composition changed')
    require(structural['tdcp_contract']['phase184_source_tdcp_huber_k_enabled'] and structural['tdcp_contract']['official_huber_k'] == 0.2 and structural['tdcp_contract']['fixed_sigma_m'] == 0.03, 'source TDCP contract changed')
...
def evaluate():
    m = verify()
    auth = read(AUTH)
    require(auth == {'phase': 228, 'allow_truth_read': True,
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_228', path)
    ...
    result = {'phase': 228, 'execution_label': 'primary agent',
