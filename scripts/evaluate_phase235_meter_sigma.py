...
MANIFEST = RECORDS / 'smartphone_r5_phase235_h_accuracy_manifest_v1.json'
AUTH = RECORDS / 'smartphone_r5_phase235_h_accuracy_authorization_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase235_h_accuracy_result_v1.json'
...
def verify():
    m = read(MANIFEST)
    require(m['phase'] == 235 and m['route'] == ROUTE, 'wrong experiment')
    ...
    structural = read(ROOT / a['phase234_result']['path'])
...
def evaluate():
    m = verify()
    auth = read(AUTH)
    require(auth == {'phase': 235, 'allow_truth_read': True,
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_235', path)
    ...
    result = {'phase': 235, 'execution_label': 'primary agent',
