...
MANIFEST = RECORDS / 'smartphone_r5_phase237_h_accuracy_manifest_v1.json'
AUTH = RECORDS / 'smartphone_r5_phase237_h_accuracy_authorization_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase237_h_accuracy_result_v1.json'
...
def verify():
    m = read(MANIFEST)
    require(m['phase'] == 237 and m['route'] == ROUTE, 'wrong experiment')
    ...
    structural = read(ROOT / a['phase236_result']['path'])
    ...
    require(all(structural['comparison_vs_phase234'][k] for k in ('epochs', 'imu_measurement_noise', 'imu_utc_fallback_offset', 'output_contract', 'raw_utc_key_contract')), 'baseline composition changed')
    require(structural['tdcp_contract']['source_tdcp_meter_sigma_requested'] and structural['tdcp_contract']['official_huber_k'] == 0.2 and structural['tdcp_contract']['fixed_sigma_m'] is None, 'source TDCP contract changed')
...
def evaluate():
    m = verify()
    auth = read(AUTH)
    require(auth == {'phase': 237, 'allow_truth_read': True,
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_237', path)
    ...
    result = {'phase': 237, 'execution_label': 'primary agent',
