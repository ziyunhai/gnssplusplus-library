...
MANIFEST = RECORDS / 'smartphone_r5_phase244_h_accuracy_manifest_v1.json'
AUTH = RECORDS / 'smartphone_r5_phase244_h_accuracy_authorization_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase244_h_accuracy_result_v1.json'
...
def verify():
    m = read(MANIFEST)
    require(m['phase'] == 244 and m['route'] == ROUTE, 'wrong experiment')
    ...
    structural = read(ROOT / a['phase243_result']['path'])
    ...
    require(structural['tdcp_contract']['source_tdcp_resl_observable_requested'] is True, 'source resL changed')
...
def evaluate():
    m = verify()
    auth = read(AUTH)
    require(auth == {'phase': 244, 'allow_truth_read': True,
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_244', path)
    ...
    result = {'phase': 244, 'execution_label': 'primary agent',
