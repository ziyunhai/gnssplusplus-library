...
MANIFEST = RECORDS / 'smartphone_r5_phase247_h_accuracy_manifest_v1.json'
AUTH = RECORDS / 'smartphone_r5_phase247_h_accuracy_authorization_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase247_h_accuracy_result_v1.json'
...
def verify():
    m = read(MANIFEST)
    require(m['phase'] == 247 and m['route'] == ROUTE, 'wrong experiment')
    ...
    structural = read(ROOT / a['phase246_result']['path'])
    ...
    require(m['candidate'] == {k: candidate[k] for k in ('path', 'sha256', 'bytes')} | {'rows': 1101}, 'candidate changed')
    require(candidate['published_rows'] == 1101, 'candidate coverage changed')
...
def evaluate():
    m = verify()
    auth = read(AUTH)
    require(auth == {'phase': 247, 'allow_truth_read': True,
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_247', path)
    ...
    result = {'phase': 247, 'execution_label': 'primary agent',
