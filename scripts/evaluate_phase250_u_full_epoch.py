...
MANIFEST = RECORDS / 'smartphone_r5_phase250_h_accuracy_manifest_v1.json'
AUTH = RECORDS / 'smartphone_r5_phase250_h_accuracy_authorization_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase250_h_accuracy_result_v1.json'
ROUTE = '2023-03-08-21-34-us-ca-mtv-u/pixel5'
BASELINE = None
...
def verify():
    m = read(MANIFEST)
    require(m['phase'] == 250 and m['route'] == ROUTE, 'wrong experiment')
    ...
    structural = read(ROOT / a['phase249_result']['path'])
    ...
    require(structural['graph']['phase217_main_motion'] == {'requested': True, 'enabled': True, 'factors': 1101, 'gap_skips': 0, 'sigma_m': 0.05, 'gap_threshold_s': 1.5}, 'main motion counts changed')
    require(structural['graph']['phase213_main_doppler'] == {'requested': True, 'enabled': True, 'factors': 12496}, 'main Doppler counts changed')
    ...
    truth_source = read(ROOT / a['truth_metadata']['path'])['routes'][ROUTE]
    require(m['truth'] == {k: truth_source[k] for k in ('path', 'sha256', 'bytes')} | {'rows': 1102}, 'truth metadata changed')
    require(m['metric_contract'] == old['metric_contract'], 'metric changed')
    require(all(structural['comparison_vs_phase246'].values()), 'estimator changed')
    require(structural['raw_utc_key_contract']['warmup_epoch_excluded'] is False, 'first epoch excluded')
    require(structural['raw_utc_key_contract']['exact_solution_epochs'] == 1102, 'incomplete raw coverage')
    candidate = structural['candidate_identity']
    require(m['candidate'] == {k: candidate[k] for k in ('path', 'sha256', 'bytes')} | {'rows': 1102}, 'candidate changed')
    require(candidate['published_rows'] == 1102, 'candidate coverage changed')
...
def evaluate():
    m = verify()
    auth = read(AUTH)
    require(auth == {'phase': 250, 'allow_truth_read': True,
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_250', path)
    ...
    result = {'phase': 250, 'execution_label': 'primary agent',
              'manifest_sha256': auth['manifest_sha256'], 'role': m['role'],
              'baseline_m': None, 'comparison': 'U development transfer; no H delta comparison', **score}
