...
MANIFEST = RECORDS / 'smartphone_r5_phase255_h_accuracy_manifest_v1.json'
AUTH = RECORDS / 'smartphone_r5_phase255_h_accuracy_authorization_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase255_h_accuracy_result_v1.json'
...
def verify():
    m = read(MANIFEST)
    require(m['phase'] == 255 and m['route'] == ROUTE, 'wrong experiment')
    ...
    structural = read(ROOT / a['phase254_result']['path'])
    ...
    require(tdcp['tdcp_only_affine_geometry_requested'] is True, 'affine selector missing')
    require(tdcp['tdcp_only_affine_factors_inserted'] == 69270 and
            tdcp['gnss_first_tdcp_only_affine_factors_inserted'] == 69270,
...
def evaluate():
    m = verify()
    auth = read(AUTH)
    require(auth == {'phase': 255, 'allow_truth_read': True,
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_255', path)
    ...
    result = {'phase': 255, 'execution_label': 'primary agent',
