...
MANIFEST = RECORDS / 'smartphone_r5_phase261_h_accuracy_manifest_v1.json'
AUTH = RECORDS / 'smartphone_r5_phase261_h_accuracy_authorization_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase261_h_accuracy_result_v1.json'
...
def verify():
    m = read(MANIFEST)
    require(m['phase'] == 261 and m['route'] == ROUTE, 'wrong experiment')
    ...
    structural = read(ROOT / a['phase260_result']['path'])
    ...
    require(all(structural['comparison_vs_phase234'][k] for k in ('gnss_first', 'epochs', 'imu_measurement_noise', 'imu_utc_fallback_offset', 'output_contract', 'raw_utc_key_contract')), 'baseline composition changed')
    ...
    require(tdcp['epoch_heading_attitude_seeds_requested'] is True and
            tdcp['epoch_heading_attitude_seeds_inserted'] == 3140,
...
def evaluate():
    m = verify()
    auth = read(AUTH)
    require(auth == {'phase': 261, 'allow_truth_read': True,
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_261', path)
    ...
    result = {'phase': 261, 'execution_label': 'primary agent',
