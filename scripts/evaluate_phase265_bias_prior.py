...
MANIFEST = RECORDS / 'smartphone_r5_phase265_h_accuracy_manifest_v1.json'
AUTH = RECORDS / 'smartphone_r5_phase265_h_accuracy_authorization_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase265_h_accuracy_result_v1.json'
...
def verify():
    m = read(MANIFEST)
    require(m['phase'] == 265 and m['route'] == ROUTE, 'wrong experiment')
    ...
    structural = read(ROOT / a['phase264_result']['path'])
    ...
    require(tdcp['omit_first_imu_bias_prior_requested'] is True and
            tdcp['first_imu_bias_priors_inserted'] == 0 and
            tdcp['first_imu_bias_priors_omitted'] == 1, 'bias prior omission changed')
...
def evaluate():
    m = verify()
    auth = read(AUTH)
    require(auth == {'phase': 265, 'allow_truth_read': True,
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_265', path)
    ...
    result = {'phase': 265, 'execution_label': 'primary agent',
