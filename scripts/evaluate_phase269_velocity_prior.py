...
MANIFEST = RECORDS / 'smartphone_r5_phase269_h_accuracy_manifest_v1.json'
AUTH = RECORDS / 'smartphone_r5_phase269_h_accuracy_authorization_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase269_h_accuracy_result_v1.json'
...
def verify():
    m = read(MANIFEST)
    require(m['phase'] == 269 and m['route'] == ROUTE, 'wrong experiment')
    ...
    structural = read(ROOT / a['phase268_result']['path'])
    ...
    require(tdcp['omit_first_imu_velocity_prior_requested'] is True and
            tdcp['first_imu_velocity_priors_inserted'] == 0 and
            tdcp['first_imu_velocity_priors_omitted'] == 1, 'velocity prior omission changed')
    require(structural['graph']['factors'] == 252383, 'graph factor count changed')
...
def evaluate():
    m = verify()
    auth = read(AUTH)
    require(auth == {'phase': 269, 'allow_truth_read': True,
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_269', path)
    ...
    result = {'phase': 269, 'execution_label': 'primary agent',
