...
MANIFEST = RECORDS / 'smartphone_r5_phase286_h_accuracy_manifest_v1.json'
AUTH = RECORDS / 'smartphone_r5_phase286_h_accuracy_authorization_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase286_h_accuracy_result_v1.json'
...
def verify():
    m = read(MANIFEST)
    require(m['phase'] == 286 and m['route'] == ROUTE, 'wrong experiment')
    ...
    structural = read(ROOT / a['phase285_result']['path'])
    ...
    require(all(structural['comparison_vs_phase234'][k] for k in ('gnss_first', 'imu_initialization', 'imu_measurement_noise', 'imu_utc_fallback_offset', 'output_contract', 'raw_utc_key_contract')), 'baseline composition changed')
    ...
    require(structural['graph']['factors'] == 252518, 'graph factor count changed')
    require(all(structural['epoch_comparison_except_p_count'].values()) and
            structural['epochs']['pseudorange_factors'] == 102050, 'epoch contract changed')
    require(tdcp['relative_height_pairs_requested'] is False and
            tdcp['relative_height_factors_inserted'] == 0, 'relative height changed')
    expected = {'requested': True, 'applied': True, 'pool': 105188, 'old': 101916,
                'new': 102050, 'recovered': 687, 'removed': 553, 'unchanged': 101363,
                'convention': 'same-run-range-clock-residual-only; fixed-measurements-sigma'}
    require(all(tdcp['pseudorange_remasking_' + k] == v for k,v in expected.items()),
            'remasking contract changed')
...
def evaluate():
    m = verify()
    auth = read(AUTH)
    require(auth == {'phase': 286, 'allow_truth_read': True,
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_286', path)
    ...
    result = {'phase': 286, 'execution_label': 'primary agent',
