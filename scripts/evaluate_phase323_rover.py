...
MANIFEST = RECORDS / 'smartphone_r5_phase323_rover_accuracy_manifest_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase323_rover_accuracy_result_v1.json'
...
def verify():
    ...
    assert s['rover_states_requested'] is True
    assert s['paired_states_requested'] is False
    assert s['zero_explicit_code_bias_requested'] is False
...
def evaluate():
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_323', path)
    ...
    result = dict(score, phase=323, role=m['role'], baseline_m=m['baseline_m'],
