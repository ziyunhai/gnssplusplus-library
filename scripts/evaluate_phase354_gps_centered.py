...
MANIFEST = RECORDS / 'smartphone_r5_phase354_gps_centered_accuracy_manifest_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase354_gps_centered_accuracy_result_v1.json'
...
def verify():
    ...
    assert s['base']['correction_applied_exactly_once']
    assert s['dense_base_smoothing_requested'] is True
    assert s['mask_only_requested'] is False and s['nonzero_correction_enabled'] is True
    assert s['gps_values_only_requested'] is True
    assert s['gps_centered_requested'] is True
    assert s['epochs']['pseudorange_factors'] == 81591
    assert s['same_process_retained_identity_check_passed'] is True
    ...
    assert m['baseline_m'] == old['baseline_m']
    return m
...
def evaluate():
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_354', path)
    ...
    result = dict(score, phase=354, role=m['role'], baseline_m=m['baseline_m'],
