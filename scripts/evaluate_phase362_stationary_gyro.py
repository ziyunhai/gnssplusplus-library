...
MANIFEST = RECORDS / 'smartphone_r5_phase362_stationary_gyro_accuracy_manifest_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase362_stationary_gyro_accuracy_result_v1.json'
...
def verify():
    ...
    assert s['main_p_cauchy_requested'] is False
    assert s['stationary_gyro_initialization']['stationary_gyro_initializer_requested'] is True
    assert s['stationary_gyro_initialization']['stationary_gyro_initializer_applied'] is True
    assert s['stationary_gyro_initialization']['stationary_gyro_blocks'] == 199
    assert s['base_enabled'] is False
    assert s['epochs']['pseudorange_factors'] == 101916
    assert s['gnss_first_baseline_aggregates_match'] is True
...
def evaluate():
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_362', path)
    ...
    result = dict(score, phase=362, role=m['role'], baseline_m=m['baseline_m'],
