...
MANIFEST = RECORDS / 'smartphone_r5_phase371_u_code_gate_accuracy_manifest_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase371_u_code_gate_accuracy_result_v1.json'
...
def verify():
    ...
    assert s['epochs']['pseudorange_factors'] == 32475
    assert s['raw_utc_key_contract']['exact_solution_epochs'] == 1102
...
def evaluate():
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_371', path)
    ...
    result = dict(score, phase=371, role=m['role'], baseline_m=m['baseline_m'],
