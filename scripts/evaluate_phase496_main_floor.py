"""Freeze, then score the completed main-only code floor candidate once; evaluation only."""
import argparse
import importlib.util
import json
from run_phase486_nhc_monitor import ROOT, digest
from verify_phase495_main_floor import main as verify_structure

RECORDS = ROOT / 'docs/use_cases/records'
MANIFEST = RECORDS / 'smartphone_r5_phase496_main_floor_accuracy_manifest_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase496_main_floor_accuracy_result_v1.json'
...
def freeze():
    ...
    for path in (directory / 'manifest.json', directory / 'completed.json',
                 ...
                 ROOT / 'scripts/verify_phase495_main_floor.py',
                 ROOT / 'scripts/evaluate_phase496_main_floor.py'):
        name = str(path.relative_to(ROOT))
        authorities[name] = dict(path=name, sha256=digest(path))
    manifest = dict(phase=496, route=old['route'], role=old['role'],
...
def evaluate():
    ...
    assert manifest['candidate']['sha256'] == '548a3dec72dea622f24770195b6b680ea24acf06f93c36f9bad7123a8065b95d'
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_496', path)
    ...
    result = dict(score, phase=496, role=manifest['role'], baseline_m=manifest['baseline_m'],
