"""Freeze, then score the completed main-only code readmission candidate once; evaluation only."""
import argparse
import importlib.util
import json
from run_phase486_nhc_monitor import ROOT, digest
from verify_phase507_readmission import main as verify_structure

RECORDS = ROOT / 'docs/use_cases/records'
MANIFEST = RECORDS / 'smartphone_r5_phase508_readmission_accuracy_manifest_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase508_readmission_accuracy_result_v1.json'
...
def freeze():
    ...
    for path in (directory / 'manifest.json', directory / 'completed.json',
                 ...
                 ROOT / 'scripts/verify_phase507_readmission.py',
                 ROOT / 'scripts/evaluate_phase508_readmission.py'):
        name = str(path.relative_to(ROOT))
        authorities[name] = dict(path=name, sha256=digest(path))
    manifest = dict(phase=508, route=old['route'], role=old['role'],
...
def evaluate():
    ...
    assert manifest['candidate']['sha256'] == 'f8996b81b9880b4c34f3949d397cb5072188c5a3882a01202158be68e0a7c59c'
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_508', path)
    ...
    result = dict(score, phase=508, role=manifest['role'], baseline_m=manifest['baseline_m'],
