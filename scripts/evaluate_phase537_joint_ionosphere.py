...
from verify_phase536_joint_ionosphere import main as verify_structure

RECORDS = ROOT / 'docs/use_cases/records'
MANIFEST = RECORDS / 'smartphone_r5_phase537_joint_ionosphere_accuracy_manifest_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase537_joint_ionosphere_accuracy_result_v1.json'
...
def freeze():
    ...
    baseline = ROOT / 'output/smartphone-r5/phase535-h-ionosphere-monitor-v1/mtv-h/opaque_solution_output.csv'
    ...
    for path in (directory / 'manifest.json', directory / 'completed.json',
                 ...
                 ROOT / 'scripts/verify_phase536_joint_ionosphere.py',
                 ROOT / 'scripts/evaluate_phase537_joint_ionosphere.py'):
        name = str(path.relative_to(ROOT))
        authorities[name] = dict(path=name, sha256=digest(path))
    manifest = dict(phase=537, route=old['route'], role=old['role'],
...
def evaluate():
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_537', path)
    ...
    result = dict(score, phase=537, role=manifest['role'], baseline_m=baseline_m,
