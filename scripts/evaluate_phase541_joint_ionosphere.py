...
MANIFEST = RECORDS / 'smartphone_r5_phase541_joint_ionosphere_accuracy_manifest_v1.json'
RESULT = RECORDS / 'smartphone_r5_phase541_joint_ionosphere_accuracy_result_v1.json'

def freeze():
    verify_structure('u')
    old = json.loads((RECORDS / 'smartphone_r5_phase247_h_accuracy_manifest_v1.json').read_text())
    directory = ROOT / 'output/smartphone-r5/phase538-joint-transfer-v1/u'
    candidate = directory / 'opaque_solution_output.csv'
    recipe = json.loads((RECORDS / 'smartphone_r5_phase480_u_comparison_manifest_v1.json').read_text())
    ...
    for path in (directory / 'manifest.json', directory / 'completed.json',
                 ...
                 ROOT / 'scripts/evaluate_phase541_joint_ionosphere.py'):
        name = str(path.relative_to(ROOT))
        authorities[name] = dict(path=name, sha256=digest(path))
    manifest = dict(phase=541, route=old['route'], role=old['role'],
                    truth=old['truth'], metric_contract=old['metric_contract'],
                    historical_baseline_m=old['baseline_m'], authority=authorities,
...
def evaluate():
    ...
    spec = importlib.util.spec_from_file_location('phase203_for_541', path)
    ...
    result = dict(score, phase=541, role=manifest['role'], baseline_m=baseline_m,
