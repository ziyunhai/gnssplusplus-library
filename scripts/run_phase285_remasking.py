"""Run the frozen Phase285 raw-only experiment once; never evaluate truth."""
...
MANIFEST = ROOT / "docs/use_cases/records/smartphone_r5_phase285_h_native_phase284_remasking_manifest_v1.json"
...
def main():
    ...
    assert "--native-source-tdcp-resl-observable" not in argv
    assert "--native-tdcp-only-affine-geometry" not in argv
    assert "--native-epoch-heading-attitude-seeds" not in argv
    assert "--native-omit-first-imu-bias-prior" not in argv
    assert "--native-omit-first-imu-velocity-prior" not in argv
    assert "--native-relative-height-pairs" not in argv
    assert "--native-pseudorange-remasking" in argv
    assert "--android-include-first-native-epoch" not in argv
