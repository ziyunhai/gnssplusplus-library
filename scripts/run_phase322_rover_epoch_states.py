"""Run the frozen Phase322 raw-only experiment once; never evaluate truth."""
...
MANIFEST = ROOT / "docs/use_cases/records/smartphone_r5_phase322_h_rover_epoch_states_manifest_v1.json"
...
def main():
    ...
    assert "--native-rover-epoch-states" in argv
    assert "--native-paired-epoch-states" not in argv
    assert "--native-base-pseudorange-compensation" not in argv
    assert "--native-base-pseudorange-source-miss-mask" not in argv
