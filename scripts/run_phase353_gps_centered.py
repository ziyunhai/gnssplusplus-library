"""Run the frozen Phase353 raw-only experiment once; never evaluate truth."""
...
MANIFEST = ROOT / "docs/use_cases/records/smartphone_r5_phase353_h_gps_centered_manifest_v1.json"
...
def main():
    ...
    for flag, name in [("--android-gnss", "android_gnss"),
                       ("--android-imu", "android_imu"), ("--nav", "nav"), ("--native-base-rinex", "base")]:
        assert argv[argv.index(flag) + 1] == manifest["inputs"][name]["path"]
    assert "--native-paired-epoch-states" in argv
    assert "--native-dense-base-smoothing" in argv
    assert "--native-base-gps-values-only-ablation" in argv
    assert "--native-base-gps-center-ablation" in argv
    assert "--native-base-pseudorange-source-miss-mask" in argv
    assert argv[argv.index("--native-base-rinex-sha256") + 1] == manifest["inputs"]["base"]["sha256"]
