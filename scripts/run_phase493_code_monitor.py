"""One raw H uncertainty diagnostic replay; no truth or weight changes."""
import json
import os
import re
import subprocess
...
def main():
    previous = ROOT / 'output/smartphone-r5/phase488-h-batch-nhc-v1/mtv-h/manifest.json'
    ...
    pins['scripts/run_phase493_code_monitor.py'] = digest(ROOT / 'scripts/run_phase493_code_monitor.py')
    for item in old['inputs'].values():
        path = ROOT / item['path']
        assert digest(path) == item['sha256'] and path.stat().st_size == item['bytes']
    argv = [arg for arg in old['argv'] if arg not in ('--native-batch-nhc', '--native-nhc-monitor')]
    assert '--native-android-sv-time-uncertainty-sigma-floor' not in argv
    directory = ROOT / 'output/smartphone-r5/phase493-h-code-monitor-v1/mtv-h'
    ...
    record = dict(phase=493, argv=argv, source_pins=pins, inputs=old['inputs'],
    ...
    with (directory / 'stdout.log').open('x') as out, (directory / 'stderr.log').open('x') as err:
        ...
        print(json.dumps({'phase': 493, 'pid': process.pid}), flush=True)
        code = process.wait()
    ...
    if code == 0:
        ...
        result['monitor'] = re.findall(r'^\[native-code-uncertainty-monitor\].*$',
