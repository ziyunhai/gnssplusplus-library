...
def main():
    previous = ROOT / 'output/smartphone-r5/phase503-h-code-monitor-v1/mtv-h/manifest.json'
    old = json.loads(previous.read_text())
    pins = {}
    for name, expected in old['source_pins'].items():
        pins[name] = digest(ROOT / name)
        if pins[name] != expected and name not in ('src/algorithms/fgo_problems.cpp', 'apps/native/gnss_fgo_imu_no_base.cpp'):
            raise RuntimeError(f'unexpected source change: {name}')
    pins['scripts/run_phase505_code_monitor.py'] = digest(ROOT / 'scripts/run_phase505_code_monitor.py')
    ...
    record = dict(phase=505, argv=argv, source_pins=pins, inputs=old['inputs'],
    ...
    with (directory / 'stdout.log').open('x') as out, (directory / 'stderr.log').open('x') as err:
        ...
        print(json.dumps({'phase': 505, 'pid': process.pid}), flush=True)
        code = process.wait()
    ...
    if code == 0:
        ...
        result['monitor'] = re.findall(r'^\[native-code-edge-shadow\].*$',
