"""One raw H main-only code-edge readmission experiment; no truth."""
import json
import os
import re
import subprocess
...
def main():
    previous = ROOT / 'output/smartphone-r5/phase505-h-code-monitor-v1/mtv-h/manifest.json'
    old = json.loads(previous.read_text())
    pins = {}
    for name, expected in old['source_pins'].items():
        pins[name] = digest(ROOT / name)
        if pins[name] != expected and name not in ('src/algorithms/fgo_problems.cpp', 'apps/native/gnss_fgo_imu_no_base.cpp', 'include/libgnss++/algorithms/fgo.hpp'):
            raise RuntimeError(f'unexpected source change: {name}')
    pins['scripts/run_phase507_code_monitor.py'] = digest(ROOT / 'scripts/run_phase507_code_monitor.py')
    for name in ('include/libgnss++/core/observation.hpp', 'include/libgnss++/algorithms/code_edge_candidates.hpp', 'tests/test_code_edge_candidates.cpp'):
        pins[name] = digest(ROOT / name)
    ...
    argv.append('--native-main-code-edge-readmission')
    ...
    record = dict(phase=507, argv=argv, source_pins=pins, inputs=old['inputs'],
    ...
    with (directory / 'stdout.log').open('x') as out, (directory / 'stderr.log').open('x') as err:
        ...
        print(json.dumps({'phase': 507, 'pid': process.pid}), flush=True)
        code = process.wait()
    ...
    if code == 0:
        ...
        result['monitor'] = re.findall(r'^\[native-main-code-edge-readmission\].*$',
                                      (directory / 'stderr.log').read_text(), re.M)
    with (directory / 'completed.json').open('x') as handle:
        json.dump(result, handle, indent=2)
    print(json.dumps(result), flush=True)
    if code: return code
    assert result['monitor'] == ['[native-main-code-edge-readmission] added=243 gnss_first_changed=0 median_changed=0']
    return 0
