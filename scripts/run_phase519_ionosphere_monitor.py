...
def main():
    previous = ROOT / 'output/smartphone-r5/phase516-h-ionosphere-monitor-v1/mtv-h/manifest.json'
    ...
    for name in ('scripts/run_phase519_ionosphere_monitor.py',
                 'include/libgnss++/algorithms/pseudorange_position_information.hpp',
                 'tests/test_pseudorange_position_information.cpp'):
        pins[name] = digest(ROOT / name)
    ...
    record = dict(phase=519, argv=argv, source_pins=pins, inputs=old['inputs'],
    ...
    with (directory / 'stdout.log').open('x') as out, (directory / 'stderr.log').open('x') as err:
        ...
        print(json.dumps({'phase': 519, 'pid': process.pid}), flush=True)
        code = process.wait()
