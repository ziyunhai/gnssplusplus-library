...
def main():
    previous = ROOT / 'output/smartphone-r5/phase519-h-ionosphere-monitor-v1/mtv-h/manifest.json'
    ...
    for name in ('scripts/run_phase522_ionosphere_monitor.py',
                 'include/libgnss++/algorithms/pseudorange_position_information.hpp',
                 'tests/test_pseudorange_position_information.cpp'):
        pins[name] = digest(ROOT / name)
    ...
    record = dict(phase=522, argv=argv, source_pins=pins, inputs=old['inputs'],
    ...
    with (directory / 'stdout.log').open('x') as out, (directory / 'stderr.log').open('x') as err:
        ...
        print(json.dumps({'phase': 522, 'pid': process.pid}), flush=True)
        code = process.wait()
    ...
    if code == 0:
        ...
        result['monitor'] = re.findall(r'^\[native-code-ionosphere-irls\].*$',
