...
def main(route):
    ...
    current_path = ROOT / 'output/smartphone-r5/phase522-h-ionosphere-monitor-v1/mtv-h/manifest.json'
    ...
    directory = ROOT / f'output/smartphone-r5/phase524-transfer-monitor-v2/{route}'
    ...
    record = dict(route=route, argv=argv, inputs=old['inputs'], source_pins=current['source_pins'],
                  ...
                  launcher_sha256=digest(ROOT / 'scripts/run_phase524_transfer_monitor.py'))
    ...
    if code == 0:
        ...
        result['graph_identical'] = s['graph'] == b['graph']
    ...
    assert code == 0
    assert result['output_identical'] and result['graph_identical'] and result['gnss_first_identical']
