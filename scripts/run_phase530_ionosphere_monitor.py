...
def main():
    previous = ROOT / 'output/smartphone-r5/phase522-h-ionosphere-monitor-v1/mtv-h/manifest.json'
    ...
    for name in ('scripts/run_phase530_ionosphere_monitor.py',
                 'src/algorithms/fgo_internal.hpp',
    ...
    record = dict(phase=530, argv=argv, source_pins=pins, inputs=old['inputs'],
    ...
    with (directory / 'stdout.log').open('x') as out, (directory / 'stderr.log').open('x') as err:
        ...
        print(json.dumps({'phase': 530, 'pid': process.pid}), flush=True)
        code = process.wait()
