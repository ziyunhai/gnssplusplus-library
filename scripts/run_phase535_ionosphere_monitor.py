"""One raw-only H baseline replay with joint ionosphere disabled-path verification."""
import json
import os
import re
import subprocess
...
def main():
    previous = ROOT / 'output/smartphone-r5/phase530-h-ionosphere-monitor-v1/mtv-h/manifest.json'
    ...
    for name, expected in old['source_pins'].items():
        ...
        if pins[name] != expected and name not in (
                ...
                'src/algorithms/fgo_internal.hpp',
                'src/algorithms/fgo_problems.cpp',
                'tests/test_tdcp_residual_state_factor.cpp',
    ...
    for name in ('scripts/run_phase535_ionosphere_monitor.py',
                 'include/libgnss++/algorithms/fgo_config.hpp',
                 'src/algorithms/fgo.cpp',
    ...
    record = dict(phase=535, argv=argv, source_pins=pins, inputs=old['inputs'],
    ...
    with (directory / 'stdout.log').open('x') as out, (directory / 'stderr.log').open('x') as err:
        ...
        print(json.dumps({'phase': 535, 'pid': process.pid}), flush=True)
        code = process.wait()
    ...
    if code == 0:
        ...
        result['monitor'] = re.findall(r'^\[native-tdcp-ionosphere-endpoints\].*$',
