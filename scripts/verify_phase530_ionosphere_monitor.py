...
def parse_residual(log, epochs):
    rows = re.findall(
        r'^\[native-code-ionosphere-residual\] supported=(\d+) unsupported=(\d+) '
        r'median_alignment=(\S+) adjacent_pairs=(\d+) same_sign_pairs=(\d+) corrections_applied=0$', log, re.M)
    if len(rows) != 1:
        raise ValueError('missing or duplicated residual monitor')
    supported, unsupported = map(int, rows[0][:2])
    alignment = float(rows[0][2])
    adjacent, same_sign = map(int, rows[0][3:])
    if supported + unsupported != epochs or not 0 <= same_sign <= adjacent <= max(0, supported-1):
        raise ValueError('invalid residual monitor counts')
    if not math.isfinite(alignment) or abs(alignment) > 1 + 1e-10:
        raise ValueError('invalid residual alignment')
    return dict(supported=supported, unsupported=unsupported, median_alignment=alignment,
                adjacent_pairs=adjacent, same_sign_pairs=same_sign)

def parse_endpoints(log, expected):
    rows = re.findall(
        r'^\[native-tdcp-ionosphere-endpoints\] valid=(\d+) invalid=(\d+) '
        r'missing_code_edges=(\d+) min=(\S+) max=(\S+) factors_changed=0$', log, re.M)
    if len(rows) != 1:
        raise ValueError('missing or duplicated endpoint monitor')
    valid, invalid, missing = map(int, rows[0][:3])
    lo, hi = map(float, rows[0][3:])
    if valid != expected or invalid or not 0 <= missing <= valid:
        raise ValueError('invalid endpoint counts')
    if not all(map(math.isfinite,(lo,hi))) or not 0 < lo <= hi:
        raise ValueError('invalid endpoint range')
    return dict(valid=valid, missing_code_edges=missing, minimum=lo, maximum=hi)

def main():
    directory = ROOT / 'output/smartphone-r5/phase530-h-ionosphere-monitor-v1/mtv-h'
    ...
    residual = parse_residual((directory / 'stderr.log').read_text(), monitor['epochs'])
    endpoints = parse_endpoints((directory / 'stderr.log').read_text(), summary['epochs']['tdcp_factors_built'])
    print(json.dumps(dict(endpoints=endpoints, residual=residual, verified=True, output_identical=True, monitor=monitor,
