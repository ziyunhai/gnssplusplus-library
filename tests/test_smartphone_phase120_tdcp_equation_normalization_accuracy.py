"""Launch-free tests for the Phase120 isolated truth-only accuracy lane."""
...
CONTRACT = ROOT / "apps/commands/benchmarks/gnss_smartphone_phase120_tdcp_equation_normalization_accuracy.py"


def load_contract():
    spec = importlib.util.spec_from_file_location("phase120_accuracy_contract_test", CONTRACT)
...
class Phase120TruthOnlyAccuracyContractTests(unittest.TestCase):
    @classmethod
    ...
    def test_freeze_and_structural_result_are_opaque_and_go(self) -> None:
        ...
        self.assertEqual(freeze["phase"], 120)
        self.assertEqual(freeze["candidate"]["candidate_count"], 1)
        self.assertFalse(freeze["candidate"]["solution_publication"])
        self.assertTrue(freeze["candidate"]["phase120_offset_boundary"]["required_already_applied_in_structural_run"])
        self.assertTrue(freeze["candidate"]["phase120_offset_boundary"]["evaluator_must_not_apply_again"])
        self.assertEqual(structural["status"], "go-phase120-official-tdcp-resl-atmosphere-cancellation-structural")
        self.assertFalse(structural["accuracy_scored"])
        self.assertEqual(structural["read_accounting"]["truth_reads"], 0)

    def test_manifest_reuses_phase118_metric_and_exact_route_order(self) -> None:
        ...
        self.assertEqual(manifest["metric_contract"]["source"], "Phase102/103/112 truth-only contract and pinned Phase82/Phase76 scorer")
    ...
    def test_payload_paths_cannot_materialize_before_authorization(self) -> None:
        freeze = self.contract.verify_freeze()
        structural = self.contract.verify_structural_result()
        with self.assertRaises(self.contract.Phase120AccuracyError):
