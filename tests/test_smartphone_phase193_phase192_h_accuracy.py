#!/usr/bin/env python3
"""Pre-truth contract tests for the Phase193 H evaluator wrapper."""
...
RUNNER_PATH = ROOT / "apps/commands/benchmarks/gnss_smartphone_phase193_phase192_h_accuracy.py"
ROUTE = "2021-08-24-20-32-us-ca-mtv-h/pixel5"


def load_runner():
    spec = importlib.util.spec_from_file_location("phase193_accuracy_runner_tests", RUNNER_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Phase193 evaluator")
...
class Phase193AccuracyContractTest(unittest.TestCase):
    ...
    def test_exact_join_rejects_missing_and_extra_keys(self):
        with self.assertRaises(RUNNER.Phase193Error):
    ...
    def test_candidate_hash_fails_before_truth_read(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            with self.assertRaisesRegex(RUNNER.Phase193Error, "candidate SHA"):
    ...
    def test_truth_hash_fails_after_candidate_but_before_score(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            with self.assertRaisesRegex(RUNNER.Phase193Error, "truth SHA"):
    ...
    def test_existing_result_rejected_before_payload_reads(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            with self.assertRaisesRegex(RUNNER.Phase193Error, "overwrite"):
