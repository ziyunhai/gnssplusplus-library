"""Pre-truth contract tests for the Phase196 H evaluator wrapper."""
...
RUNNER_PATH = ROOT / "apps/commands/benchmarks/gnss_smartphone_phase196_phase195_h_accuracy.py"
ROUTE = "2021-08-24-20-32-us-ca-mtv-h/pixel5"


def load_runner():
    spec = importlib.util.spec_from_file_location("phase196_accuracy_runner_tests", RUNNER_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Phase196 evaluator")
...
class Phase196AccuracyContractTest(unittest.TestCase):
    ...
    def test_exact_join_rejects_missing_and_extra_keys(self):
        with self.assertRaises(RUNNER.Phase196Error):
    ...
    def test_candidate_hash_fails_before_truth_read(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            with self.assertRaisesRegex(RUNNER.Phase196Error, "candidate"):
                RUNNER.score_payloads(candidate_path, bad_pin, Path(truth_pin["path"]), truth_pin, P76, P74, route=ROUTE)
    ...
    def test_truth_hash_fails_after_candidate_but_before_score(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            with self.assertRaisesRegex(RUNNER.Phase196Error, "truth"):
