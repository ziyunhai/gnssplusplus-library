"""Pre-truth contract tests for the Phase199 H evaluator wrapper."""
...
RUNNER_PATH = ROOT / "apps/commands/benchmarks/gnss_smartphone_phase199_phase198_h_accuracy.py"
ROUTE = "2021-08-24-20-32-us-ca-mtv-h/pixel5"


def load_runner():
    spec = importlib.util.spec_from_file_location("phase199_accuracy_runner_tests", RUNNER_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Phase199 evaluator")
...
class Phase199AccuracyContractTest(unittest.TestCase):
    def score(self, candidate: bytes, truth: bytes, candidate_rows: int | None = None, truth_rows: int | None = None, route: str = ROUTE):
        with tempfile.TemporaryDirectory() as raw:
            ...
            return RUNNER.score_payloads(
                ...
                Path(truth_pin["path"]), truth_pin,
                route=route,
            )

    def test_manifest_verifies_without_payload_reads(self):
        result = RUNNER.verify_pre_truth()
        self.assertEqual(result["candidate_payload_reads"], 0)
        self.assertEqual(result["truth_payload_reads"], 0)
        self.assertEqual(result["accuracy_calculations"], 0)
    ...
    def test_exact_join_rejects_missing_and_extra_keys(self):
        with self.assertRaises(RUNNER.Phase199Error):
            self.score(candidate_payload([(10000, 0.0, 0.0)]), truth_payload([(10000, 0.0, 0.0), (20000, 0.0, 0.0)]))
        with self.assertRaises(RUNNER.Phase199Error):
            self.score(candidate_payload([(10000, 0.0, 0.0), (20000, 0.0, 0.0)]), truth_payload([(10000, 0.0, 0.0)]))

    def test_no_interpolation_or_hold_is_enforced(self):
        with self.assertRaises(RUNNER.Phase199Error):
            self.score(candidate_payload([(10000, 0.0, 0.0), (30000, 0.0, 0.0)]), truth_payload([(10000, 0.0, 0.0), (20000, 0.0, 0.0), (30000, 0.0, 0.0)]))

    def test_pixel5_offset_is_not_reapplied(self):
        result = self.score(candidate_payload([(10000, 37.0, -122.0)]), truth_payload([(10000, 37.0, -122.0)]))
    ...
    def test_finite_earth_and_duplicate_key_contracts(self):
        with self.assertRaises(Exception):
            P74._parse_submission(candidate_payload([(10000, 91.0, 0.0)]), ROUTE)
    ...
    def test_velocity_gate_is_reported_without_dropping_rows(self):
        result = self.score(
            candidate_payload([(1000, 0.0, 0.0), (2000, 0.0, 0.001)]),
            truth_payload([(1000, 0.0, 0.0), (2000, 0.0, 0.0)]),
        )
    ...
    def test_candidate_hash_fails_before_truth_read(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            truth_pin = {"path": str(root / "ground_truth.csv"), "sha256": "1" * 64, "bytes": 1, "rows": 1}
            with self.assertRaisesRegex(RUNNER.Phase199Error, "candidate"):
                RUNNER.score_payloads(candidate_path, bad_pin, Path(truth_pin["path"]), truth_pin, route=ROUTE)
    ...
    def test_truth_hash_fails_after_candidate_but_before_score(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            with self.assertRaisesRegex(RUNNER.Phase199Error, "truth"):
                RUNNER.score_payloads(candidate_path, candidate_pin, truth_path, truth_pin, route=ROUTE)
    ...
    def test_existing_result_rejected_before_payload_reads(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            with self.assertRaisesRegex(Exception, "overwrite"):
                RUNNER.score_payloads(candidate, candidate_pin, truth, truth_pin, route=ROUTE, result_path=existing)

    def test_kernel_accepts_another_explicit_route(self):
