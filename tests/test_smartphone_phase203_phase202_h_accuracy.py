"""Pre-truth contract tests for the Phase203 H evaluator wrapper."""
...
RUNNER_PATH = ROOT / "apps/commands/benchmarks/gnss_smartphone_phase203_phase202_h_accuracy.py"
ROUTE = "2021-08-24-20-32-us-ca-mtv-h/pixel5"


def load_runner():
    spec = importlib.util.spec_from_file_location("phase203_accuracy_runner_tests", RUNNER_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load Phase203 evaluator")
...
def candidate_payload(rows: list[tuple[int, float, float]], phone: str = ROUTE) -> bytes:
    ...
    lines.extend(
        f"{phone},{timestamp},{latitude},{longitude}"
...
def truth_payload(rows: list[tuple[int, float, float]], extra: bool = False) -> bytes:
    header = (
        "UnixTimeMillis,LatitudeDegrees,LongitudeDegrees,Unused"
        if extra
        else "UnixTimeMillis,LatitudeDegrees,LongitudeDegrees"
    )
    lines = [header]
    for timestamp, latitude, longitude in rows:
        lines.append(
            f"{timestamp},{latitude},{longitude},ok"
            if extra
            else f"{timestamp},{latitude},{longitude}"
        )
    return ("\n".join(lines) + "\n").encode()
...
class Phase203AccuracyContractTest(unittest.TestCase):
    def score(
        self,
        candidate: bytes,
        truth: bytes,
        candidate_rows: int | None = None,
        truth_rows: int | None = None,
        route: str = ROUTE,
    ):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            candidate_pin = pin(
                root / "opaque_solution_output.csv",
                candidate,
                candidate_rows or len(candidate.splitlines()) - 1,
            )
            truth_pin = pin(
                root / "ground_truth.csv",
                truth,
                truth_rows or len(truth.splitlines()) - 1,
            )
    ...
    def test_metric_exact_haversine_linear_percentiles_and_full_domain(self):
        ...
        candidate = [
            (timestamp, 0.0, 0.001 * i)
            for i, (timestamp, _, _) in enumerate(truth)
        ]
    ...
    def test_exact_join_rejects_missing_and_extra_keys(self):
        with self.assertRaises(RUNNER.Phase203Error):
            self.score(
                candidate_payload([(10000, 0.0, 0.0)]),
                truth_payload([(10000, 0.0, 0.0), (20000, 0.0, 0.0)]),
            )
        with self.assertRaises(RUNNER.Phase203Error):
            self.score(
                candidate_payload([(10000, 0.0, 0.0), (20000, 0.0, 0.0)]),
                truth_payload([(10000, 0.0, 0.0)]),
            )

    def test_no_interpolation_or_hold_is_enforced(self):
        with self.assertRaises(RUNNER.Phase203Error):
            self.score(
                candidate_payload([(10000, 0.0, 0.0), (30000, 0.0, 0.0)]),
                truth_payload(
                    [(10000, 0.0, 0.0), (20000, 0.0, 0.0), (30000, 0.0, 0.0)]
                ),
            )
    ...
    def test_finite_earth_and_duplicate_key_contracts(self):
        with self.assertRaises(Exception):
            self.score(
                candidate_payload([(10000, 91.0, 0.0)]),
                truth_payload([(10000, 0.0, 0.0)]),
            )
        duplicate = (
        ...
        with self.assertRaises(Exception):
            self.score(duplicate, truth_payload([(10000, 0.0, 0.0)]))
        duplicate_truth_header = (
            "UnixTimeMillis,LatitudeDegrees,LongitudeDegrees,LatitudeDegrees\n"
            "10000,0,0,0\n"
        ).encode()
        with self.assertRaises(Exception):
            self.score(candidate_payload([(10000, 0.0, 0.0)]), duplicate_truth_header)
        nonfinite_truth = (
            "UnixTimeMillis,LatitudeDegrees,LongitudeDegrees\n"
            "10000,nan,0\n"
        ).encode()
        with self.assertRaises(Exception):
            self.score(candidate_payload([(10000, 0.0, 0.0)]), nonfinite_truth)
    ...
    def test_candidate_hash_fails_before_truth_read(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            truth_pin = {
                "path": str(root / "ground_truth.csv"),
                "sha256": "1" * 64,
                "bytes": 1,
                "rows": 1,
            }
            with self.assertRaisesRegex(RUNNER.Phase203Error, "candidate"):
                RUNNER.score_payloads(
                    candidate_path,
                    bad_pin,
                    Path(truth_pin["path"]),
                    truth_pin,
