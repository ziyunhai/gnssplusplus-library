...
PATH = Path(__file__).resolve().parents[1] / 'scripts/evaluate_phase223_combined_motion_doppler.py'
spec = importlib.util.spec_from_file_location('phase223_tests', PATH)
...
def test_verify_opens_metadata_only():
    ...
    with patch.object(Path, 'open', guarded):
        assert runner.verify()['phase'] == 223
    assert opened
