...
PATH = Path(__file__).resolve().parents[1] / 'scripts/evaluate_phase228_source_tdcp.py'
spec = importlib.util.spec_from_file_location('phase228_tests', PATH)
...
def test_verify_opens_metadata_only():
    ...
    with patch.object(Path, 'open', guarded):
        assert runner.verify()['phase'] == 228
    assert opened
