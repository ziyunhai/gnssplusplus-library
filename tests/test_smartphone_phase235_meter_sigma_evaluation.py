...
PATH = Path(__file__).resolve().parents[1] / 'scripts/evaluate_phase235_meter_sigma.py'
spec = importlib.util.spec_from_file_location('phase235_tests', PATH)
...
def test_verify_opens_metadata_only():
    ...
    with patch.object(Path, 'open', guarded):
        assert runner.verify()['phase'] == 235
    assert opened
