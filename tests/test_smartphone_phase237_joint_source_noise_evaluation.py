...
PATH = Path(__file__).resolve().parents[1] / 'scripts/evaluate_phase237_joint_source_noise.py'
spec = importlib.util.spec_from_file_location('phase237_tests', PATH)
...
def test_verify_opens_metadata_only():
    ...
    with patch.object(Path, 'open', guarded):
        assert runner.verify()['phase'] == 237
    assert opened
