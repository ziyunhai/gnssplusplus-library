...
PATH = Path(__file__).resolve().parents[1] / 'scripts/evaluate_phase244_source_resl.py'
spec = importlib.util.spec_from_file_location('phase244_tests', PATH)
...
def test_verify_opens_metadata_only():
    ...
    with patch.object(Path, 'open', guarded):
        assert runner.verify()['phase'] == 244
    assert opened
