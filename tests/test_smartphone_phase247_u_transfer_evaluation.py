...
PATH = Path(__file__).resolve().parents[1] / 'scripts/evaluate_phase247_u_transfer.py'
spec = importlib.util.spec_from_file_location('phase247_tests', PATH)
...
def test_verify_opens_metadata_only():
    ...
    with patch.object(Path, 'open', guarded):
        assert runner.verify()['phase'] == 247
    assert opened
