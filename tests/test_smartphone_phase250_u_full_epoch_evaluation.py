...
PATH = Path(__file__).resolve().parents[1] / 'scripts/evaluate_phase250_u_full_epoch.py'
spec = importlib.util.spec_from_file_location('phase250_tests', PATH)
...
def test_verify_opens_metadata_only():
    ...
    with patch.object(Path, 'open', guarded):
        assert runner.verify()['phase'] == 250
    assert opened
