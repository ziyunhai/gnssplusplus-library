...
PATH = Path(__file__).resolve().parents[1] / 'scripts/evaluate_phase269_velocity_prior.py'
spec = importlib.util.spec_from_file_location('phase269_tests', PATH)
...
def test_verify_opens_metadata_only():
    ...
    with patch.object(Path, 'open', guarded):
        assert runner.verify()['phase'] == 269
    assert opened
...
def test_unexpected_affine_insertion_rejected(field):
    ...
    def changed(path):
        value = real(path)
        if path.name == "smartphone_r5_phase268_h_native_phase267_velocity_prior_result_v1.json":
...
@pytest.mark.parametrize('field,value', [('first_imu_velocity_priors_inserted', 1),
                                         ('first_imu_velocity_priors_omitted', 0)])
