...
PATH = Path(__file__).resolve().parents[1] / 'scripts/evaluate_phase286_remasking.py'
spec = importlib.util.spec_from_file_location('phase286_tests', PATH)
runner = importlib.util.module_from_spec(spec)
spec.loader.exec_module(runner)


@pytest.mark.parametrize('field', ['pool', 'old', 'new', 'recovered', 'removed', 'unchanged'])
def test_changed_remasking_count_rejected(field):
    real = runner.read
    def changed(path):
        value = real(path)
        if path.name == 'smartphone_r5_phase285_h_native_phase284_remasking_result_v1.json':
            value['tdcp_contract']['pseudorange_remasking_' + field] += 1
        return value
    with patch.object(runner, 'read', changed), pytest.raises(ValueError, match='remasking contract'):
        runner.verify()
...
def test_verify_opens_metadata_only():
    ...
    with patch.object(Path, 'open', guarded):
        assert runner.verify()['phase'] == 286
    assert opened
